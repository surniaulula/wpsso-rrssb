	...
	class WpssoRrssbSubmenuShareEmail {
		...
		public function __construct( &$plugin ) {
			...
			$this->p->util->add_plugin_filters( $this, array(
				'rrssb_share_email_rows' => 4,
			) );
		}

		public function filter_rrssb_share_email_rows( $table_rows, $form, $network, $submenu_obj ) {

			$utm_source_label = sprintf( _x( 'UTM Source for %s', 'option label', 'wpsso-rrssb' ), 'Email' );
...
if ( ! class_exists( 'WpssoRrssbShareEmail' ) ) {
	...
	<a href="mailto:?subject=Share:%20%%email_title%%&body=%%email_excerpt%%%0D%0A%0D%0ARead%20more%20at%20%%sharing_short_url%%%0D%0A" class="wp-block-file__button">
		...
		public function get_html( $mod, $atts ) {
			...
			$atts[ 'email_title' ] = $this->p->page->get_caption( $mod, $md_key = 'email_title', $type = 'title',
				$max_len = 0, $num_hashtags = false, $do_encode = false );

			$atts[ 'email_excerpt' ] = $this->p->page->get_caption( $mod, $md_key = 'email_desc', $type = 'both',
