...
if ( ! class_exists( 'WpssoRrssbSubmenuSharePinterest' ) ) {
	...
	class WpssoRrssbSubmenuSharePinterest {
		...
		public function filter_image_dimensions_general_rows( $table_rows, $form ) {
			...
			$tooltip_msg = sprintf( __( 'The image dimensions the Pinterest Pin It button will share (defaults is %s).', 'wpsso-rrssb' ), $def_dimensions ) . ' ' . __( 'Images for Facebook / Open Graph are usually cropped, whereas images for Pinterest often look better in their original uncropped aspect ratio.', 'wpsso-rrssb' );
		...
		public function filter_rrssb_share_pinterest_rows( $table_rows, $form, $submenu ) {
			...
			$table_rows[] = $form->get_tr_hide_img_dim( 'basic', 'pin_img' ) .
			$form->get_th_html( _x( 'Image Size', 'option label', 'wpsso-rrssb' ) ) . 
			'<td>' . $form->get_input_image_dimensions( 'pin_img' ) . '</td>';
