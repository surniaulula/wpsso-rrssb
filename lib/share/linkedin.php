	...
	class WpssoRrssbSubmenuShareLinkedin {
		...
		public function __construct( &$plugin ) {
			...
			$this->p->util->add_plugin_filters( $this, array(
				'rrssb_share_linkedin_rows' => 4,
			) );
		}

		public function filter_rrssb_share_linkedin_rows( $table_rows, $form, $network, $submenu_obj ) {

			$utm_source_label = sprintf( _x( 'UTM Source for %s', 'option label', 'wpsso-rrssb' ), 'LinkedIn' );

			$table_rows[] = '' .
				$form->get_th_html( _x( 'Show Button in', 'option label', 'wpsso-rrssb' ) ) .
				'<td>' . $submenu_obj->show_on_checkboxes( 'linkedin' ) . '</td>';

			$table_rows[] = '' .
				$form->get_th_html( _x( 'Preferred Order', 'option label', 'wpsso-rrssb' ) ) .
				'<td>' . $form->get_select( 'linkedin_button_order', range( 1, count( $submenu_obj->share ) ) ) . '</td>';

			$table_rows[] = $form->get_tr_hide( $in_view = 'basic', 'linkedin_utm_source' ) .
				$form->get_th_html( $utm_source_label ) .
				'<td>' . $form->get_input( 'linkedin_utm_source' ) . '</td>';

			$table_rows[] = $form->get_tr_hide( $in_view = 'basic', 'linkedin_caption_max_len' ) .
			...
			$table_rows[] = $form->get_tr_hide( $in_view = 'basic', 'linkedin_rrssb_html' ) .
